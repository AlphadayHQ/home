...
| 23 | **14 of 66 boards filter content by a slug the tag taxonomy does not use**, so each renders an empty feed — in the product, not only in SEO. The board slug is used verbatim as the tag filter, but the taxonomy disagrees: Polygon is `matic-network` (a pre-rebrand name, while the board is `polygon` and the token is now `POL`), Avalanche is `avalanche-2` (a disambiguated coin id), and five more are unhyphenated (`ethereumclassic`, `rocketpool`, `thegraph`, `liquidstaking`, `vitalikbuterin`). Worldcoin alone hides 5,173 articles. Audited by `scripts/audit-tag-coverage.mjs` | §5.8, content C3 |

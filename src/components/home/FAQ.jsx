...
const Card = ({
    ...
    return (
        <div className="bg-black rounded-xl p-4 md:p-8 transform-all duration-300">
            <div onClick={() => handleToggle(index)} className={`text-platinum flex justify-between items-center ${ toggleDrawer == index ? "mb-4" : "mb-0"}`}>
                <h4 className="font-bold text-sm md:text-xl">{question}</h4>
                <span className={`flex justify-center items-center w-[25px] h-[25px] rounded-full border ${toggleDrawer == index ? "border-aluminium" :"border-blue"}`}>
                    <i class={`${toggleDrawer == index ? "ri-subtract-line" : "ri-add-fill"} `}></i>
                </span>
            </div>
            ...
            {   
                ...
                answer.map((el, index) => {
                    return(
                        <p key={index} className={`text-aluminium px-4 text-sm md:px-8 md:text-base ${index == answer.length-1 && "mt-4"} transform-all duration-300`}>
                            {el}
                        </p>
                    )
                })
